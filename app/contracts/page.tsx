...
export default function ContractsPage() {
  ...
  return (
    <main className="contracts-page" style={{ minHeight: "100vh", background: "#f5f4ef", color: "#111" }}>
      <section style={{ background: "#111", color: "#fff", padding: "55px 6vw 50px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ fontSize: 12, letterSpacing: 2, fontWeight: 700, color: "#aaa", marginBottom: 14 }}>WOMEN&apos;S FOOTBALL MARKET</div>
          <h1 style={{ fontSize: 46, lineHeight: 1.05, margin: 0, fontWeight: 800 }}>Contract Intelligence</h1>
          <p style={{ margin: "18px 0 0", maxWidth: 720, color: "#ccc", fontSize: 17, lineHeight: 1.6 }}>Track active deals, contract expiry windows, salary information and confidence across the database.</p>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 20px 0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14 }}>
          {[
            ["Active Contracts", activeContracts.length.toString()],
            ["Expiring ≤90 Days", expiring90.length.toString()],
            ["Expiring ≤6 Months", (expiring90.length + expiring180.length).toString()],
            ["Expiring ≤1 Year", (expiring90.length + expiring180.length + expiring365.length).toString()],
            ["Known Payroll", formatSalary(knownPayroll)],
          ].map(([label, value]) => <div key={label} style={{ background: "#fff", border: "1px solid #e5e5e5", borderRadius: 12, padding: "18px 20px" }}><div style={{ fontSize: 11, color: "#777", fontWeight: 700, letterSpacing: .8 }}>{label}</div><div style={{ fontSize: 25, fontWeight: 800, marginTop: 7 }}>{loading ? "—" : value}</div></div>)}
        </div>
      </section>

      <section style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px 60px" }}>
        <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: 12, padding: 20, marginBottom: 18 }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 12 }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search player, club, league, position..." style={{ padding: "12px 14px", border: "1px solid #ccc", borderRadius: 8, fontSize: 14, minWidth: 0 }} />
            <select value={expiry} onChange={e => setExpiry(e.target.value)} style={{ padding: "12px 14px", border: "1px solid #ccc", borderRadius: 8, background: "#fff", fontSize: 14 }}>
              <option value="All">All Expiry Windows</option><option value="90">Next 90 Days</option><option value="180">3–6 Months</option><option value="365">6–12 Months</option><option value="long">12+ Months</option><option value="expired">Expired</option><option value="unknown">Unknown</option>
            </select>
            <select value={status} onChange={e => setStatus(e.target.value)} style={{ padding: "12px 14px", border: "1px solid #ccc", borderRadius: 8, background: "#fff", fontSize: 14 }}>{statuses.map(v => <option key={v} value={v}>{v === "All" ? "All Statuses" : statusLabel(v)}</option>)}</select>
            <select value={league} onChange={e => setLeague(e.target.value)} style={{ padding: "12px 14px", border: "1px solid #ccc", borderRadius: 8, background: "#fff", fontSize: 14 }}>{leagues.map(v => <option key={v} value={v}>{v === "All" ? "All Leagues" : v}</option>)}</select>
          </div>
          {hasFilters && <button onClick={clearFilters} style={{ marginTop: 12, border: 0, background: "transparent", padding: 0, cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#555" }}>Clear filters</button>}
        </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1.45fr .9fr 1.15fr 1.15fr .9fr", gap: 12, padding: "14px 18px", background: "#f7f7f7", borderBottom: "1px solid #ddd", fontSize: 11, fontWeight: 700, color: "#666", textTransform: "uppercase", letterSpacing: .6 }}>
            <div>Player</div><div>Club</div><div>Status</div><div>Expiry</div><div>Salary</div><div>Confidence</div>
          </div>
          {loading ? <div style={{ padding: 35, textAlign: "center", color: "#777" }}>Loading...</div> : filteredContracts.length === 0 ? <div style={{ padding: 35, textAlign: "center", color: "#777" }}>No contracts found.</div> : filteredContracts.map(c => {
            ...
            return <Link key={c.id} href={`/players/${c.player_id}`} style={{ display: "grid", gridTemplateColumns: "2fr 1.45fr .9fr 1.15fr 1.15fr .9fr", gap: 12, padding: "16px 18px", borderBottom: "1px solid #eee", textDecoration: "none", color: "#111", alignItems: "center" }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700 }}>{formatSalary(c.annual_salary_usd)}</div>
              <div style={{ fontSize: 12, color: "#555" }}>{c.confidence ? statusLabel(c.confidence) : "Unknown"}</div>
            </Link>;
          })}
        </div>
      </section>

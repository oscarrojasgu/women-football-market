...
type ClubContext={id:string;name:string;country:string|null;league:string|null;activePlayers:number;expiringContracts:number;unknownSalary:number;positionMix:[string,number][]};
...
export default function GlobalDiscoveryPage(){
 ...
 useEffect(()=>{if(!id)return;let mounted=true;(async()=>{const auth=await supabase.auth.getUser();if(!mounted)return;const uid=auth.data.user?.id||null;setUserId(uid);const [pr,pl,pg,si,co,mv,pa,listResult]=await Promise.all([
  ...
  setProfile({...pr.data,criteria:normalize(pr.data.criteria)} as Profile);const linkedClubId=(pr.data as any)?.club_id||null;if(linkedClubId){const [{data:club},{data:cc}]=await Promise.all([supabase.from("clubs").select("id,name,country,league").eq("id",linkedClubId).maybeSingle(),supabase.from("contracts").select("player_id,end_date,annual_salary_usd,status").eq("club_id",linkedClubId).eq("status","active")]);if(club){const ids=[...new Set((cc||[]).map((x:any)=>x.player_id))];let mix:[string,number][]=[];if(ids.length){const {data:ps}=await supabase.from("players").select("id,position").in("id",ids);const counts=new Map<string,number>();(ps||[]).forEach((p:any)=>{const k=p.position||"Unknown";counts.set(k,(counts.get(k)||0)+1)});mix=Array.from(counts.entries()).sort((a,b)=>b[1]-a[1])}const now=new Date(),cutoff=new Date(now);cutoff.setDate(cutoff.getDate()+180);setClubContext({id:club.id,name:club.name,country:club.country,league:club.league,activePlayers:(cc||[]).length,expiringContracts:(cc||[]).filter((x:any)=>x.end_date&&new Date(x.end_date+"T23:59:59")>=now&&new Date(x.end_date+"T23:59:59")<=cutoff).length,unknownSalary:(cc||[]).filter((x:any)=>x.annual_salary_usd==null).length,positionMix:mix})}}setLists((listResult.data||[]) as {id:string;name:string;status:string}[]);if(listResult.data?.length)setSelectedListId(listResult.data[0].id);setPlayers((pl.data||[]) as Player[]);const peerMap=new Map<string,GlobalPeer>();for(const row of (pg.data||[]) as GlobalPeer[])peerMap.set(`${row.player_id}|${row.season}`,row);setGlobalPeers(peerMap);setIntel((si.data||[]) as Intel[]);setContracts((co.data||[]) as Contract[]);setValues((mv.data||[]) as Value[]);

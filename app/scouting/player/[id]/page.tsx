...
export default async function ScoutingReportPage({ params, searchParams }: PageProps) {
  ...
  const [{ data: intelligence }, { data: contracts }, { data: values }, { data: peers }, { data: memberships }, { data: notes }, { data: pipelines }, { data: scoutingProfile }, { data: sourceStats }] = await Promise.all([
    ...
    supabase.from("contracts").select("status,start_date,end_date,annual_salary_usd,weekly_salary_usd,confidence,club:clubs(name),source:sources(id,publisher,reliability,published_at,url)").eq("player_id", id).order("start_date", { ascending: false }),
    supabase.from("market_values").select("market_value_usd,valuation_date,confidence,source:sources(id,publisher,reliability,published_at,url)").eq("player_id", id).order("valuation_date", { ascending: false }).limit(1),
  ...
  const statSources = Array.from(new Map(statsEvidence.map(row => [row.source_id, row.source] as [string, any]).filter(([sourceId, source]) => sourceId && source)).values()) as any[];

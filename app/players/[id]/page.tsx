...
export default async function PlayerPage({ params }: PlayerPageProps) {
  ...
  const activeContract = contracts.find(c => c.status?.toLowerCase() === "active") || null;
    ...
    <main className="player-page" style={{ minHeight: "100vh", background: "#f5f4ef", color: "#111" }}>
      ...
      <div className="player-content" style={{ maxWidth: 1200, margin: "0 auto", padding: "22px 20px 60px" }}>
        <div className="player-overview-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 12 }}>
          <div style={card}><div style={label}>Current Club</div><div style={{ marginTop: 7, fontWeight: 750 }}>{currentClub ? <Link href={`/clubs/${currentClub.id}`} style={{ color: "#111" }}>{currentClub.name}</Link> : "—"}</div><div style={{ color: "#888", fontSize: 12, marginTop: 3 }}>{currentClub?.league || (currentClub ? `Linked from ${currentClubSource}` : "Club not recorded")}</div></div>
          <div style={card}><div style={label}>Contract Expiry</div><div style={{ marginTop: 7, fontWeight: 750 }}>{currentContract?.end_date ? dateText(currentContract.end_date) : "No active contract"}</div><div style={{ color: "#888", fontSize: 12, marginTop: 3 }}>{currentContract ? `${titleCase(currentContract.confidence)} confidence` : "Contract history retained below"}</div></div>
